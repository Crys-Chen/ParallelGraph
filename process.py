        ...
        for entry in log_entries:
            ...
            if self_match_inter is None:
                ...
                if "," in index:
                    ...
                    insts.append(OriginalInstruction(current_step, type, [element]))
                else:
                    vector = VectorElement(self_match_initial.group(2), int(index))
                    insts.append(OriginalInstruction(current_step, type, [vector]))
                ...
                insts.append(OriginalInstruction(current_step, type, inputs, previous, depend))

        return insts
    
def SIMD(filename):
    original_insts = read_file(filename)
    combined_insts = []
    print("Original: \n", original_insts)
    for original_inst in original_insts:
        for combined_inst in combined_insts:
            if combined_inst.try_absorb(combined_insts, original_inst):
                break
        else:
            combined_insts.append(CombinedInstruction(len(combined_insts), original_inst, combined_insts))
    print("Combined: \n", combined_insts)
